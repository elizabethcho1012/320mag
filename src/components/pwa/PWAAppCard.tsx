...
export function PWAAppCard({ app }: PWAAppCardProps) {
  // Use first screenshot or icon for the mockup
  const mockupImage = app.screenshots?.[0] || app.icon;

  // Check if app was recently updated (within last 7 days)
  ...
  return (
    <Link to={`/apps/${app.id}`}>
      <div className="rounded-2xl p-2 hover:bg-gray-100 transition-all duration-300 group relative overflow-hidden py-5 bg-white">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-0.5">
                  ...
                  <span className="text-[8px] text-gray-600">{app.downloadCount >= 1000 ? `${(app.downloadCount / 1000).toFixed(0)}K+` : app.downloadCount}</span>
                </div>
              </div>
            </div>
            {app.icon && (
